...
t_model get_model(void)
{
	...
	while (k + l <= j)
	{
		if (ordre[k + l].w == -1)
		{
			model.triangles[k] = add_triangle(coord[(int)ordre[k + l].x], coord[(int)ordre[k + l].y], coord[(int)ordre[k + l].z]);
	...
	while (++k <= j)
	{
		...
		{
			if (ordre[k].w == -1 && l <= 2)
				printf("triangles[%d].p[%d] = %f %f %f \n",k, l, model.triangles[k].p[l].x, model.triangles[k].p[l].y, model.triangles[k].p[l].z);
			else if (ordre[k].w != -1 && l <= 3)
				printf("planes[%d].p[%d] = %f %f %f \n",k, l, model.planes[k].plane[l].x, model.planes[k].plane[l].y, model.planes[k].plane[l].z);
		}
	}*/
