...
t_info	cylinder_collision(t_ray ray, const t_cylinder *cylinder)
{
	...
	b = 2.0f * (vector3_dot(oc, ray.direction) - vector3_dot(ray.direction, \
			cylinder->normal) * vector3_dot(oc, cylinder->normal));
	c = vector3_dot(oc, oc) - pow(vector3_dot(oc, cylinder->normal), 2)
