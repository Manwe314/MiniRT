...
/*void hook(void *param)
{
	...
	while (++x < minirt->width)
	{
		...
		while (++y < minirt->height)
		{
			...
			t_vector4 blurred_color = color[x][y];
			t_vector4 accumulated_color = vector4_multiply_float(blurred_color , 1.0f / (float)i);
			...
			//accumulated_color = saturate(accumulated_color, i, x, y, minirt);
